...
function Fizzbuzz(inputValue){
	...
	if (isNaN(newValueTest)) {
		alert('Not a number, please enter a number only no demicals or negatives.'); // Not a valid number try again
	}

	else if (newValueTest % 1 !== 0) { //Apply a restriction rule stated on the assignment and also example on stackoverflow.
		alert('Do not use decimals. Please enter a whole number.');//Apply alert to let the user know
	}
